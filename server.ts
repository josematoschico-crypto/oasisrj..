...
async function startServer() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    const { phoneNumber, pin } = req.body;
    console.log(`[Server] Login attempt - Phone: ${phoneNumber || "MISSING"}, PIN: ${pin ? "****" : "MISSING"}`);
    ...
    try {
      ...
      // Search for user with this phone number and PIN (or just PIN if phone is missing)
      let snapshot;
      try {
        snapshot = await withDbFallback(async (database) => {
          let query = database.collection("users").where("pin", "==", pin);
          if (phoneNumber) {
  ...
  app.post("/api/admin/elevate", async (req, res) => {
    ...
    // Universal password "5023" for institutional access
    if (pin !== "5023") {
